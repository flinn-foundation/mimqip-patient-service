...
import org.flinnfoundation.old.dao.beans.AppUserRoleBean;
import org.flinnfoundation.old.dao.beans.LabTestBean;
import org.flinnfoundation.old.dao.beans.ProgressNoteTagBean;
import org.flinnfoundation.old.dao.beans.TreatmentGroupBean;
import org.flinnfoundation.old.dao.beans.request.*;
import org.flinnfoundation.old.dao.beans.response.*;
import org.flinnfoundation.old.dao.dao.imp.*;
...
public class DaoAppManager extends AbstractBaseDao
{
	...
	public ResponseFacilityBean getFacility(int facilityid) throws Exception
	{
		...
		if (facilityid > 0)
		{
			...
			List<ResponseFacilityBean> facilities = new FacilityDaoImp().find(facreq, null, connection);
	...
	public ResponseContainerBean updateFacility(RequestContainerBean input, ResponseSessionContainerBean session)
	{
		...
		//Force value of "0000-00-00 00:00:00" to avoid null exception in org.flinnfoundation.old.dao.dao update
	...
	public LabTestBean findLabTest(int labtestid) throws Exception
	{
		...
		{
			connection = getConnection(); //Get connection
			List<LabTestBean> labtests = (new LabDaoImp().findTest(labtest, null, connection));
	...
	public TreatmentGroupBean getTreatmentGroup(int treatmentgroupid) throws Exception
	{
		...
		{
			connection = getConnection(); //Get connection
			List<TreatmentGroupBean> groups = (new TreatmentDaoImp().findGroup(group, null, connection));
	...
	public ResponsePrescriptionBean getPrescription(int prescriptionid) throws Exception
	{
		...
		if (prescriptionid > 0)
		{
			...
			RequestPrescriptionBean beanreq = new RequestPrescriptionBean();
			beanreq.setPrescriptionid(prescriptionid);
			List<ResponsePrescriptionBean> prescriptions = new PrescriptionDaoImp().find(beanreq, null, connection);
