package flinn.old.dao.recommend.dao;
...
import flinn.old.dao.dao.AbstractBaseDao;
import flinn.old.dao.dao.AuthcodeDao;
...
import flinn.old.dao.recommend.beans.request.RequestMessageBean;
import flinn.old.dao.recommend.beans.request.RequestRuleBean;
import flinn.old.dao.recommend.beans.request.RequestSettingBean;
import flinn.old.dao.recommend.beans.response.*;
import flinn.old.dao.recommend.dao.imp.MessageDaoImp;
import flinn.old.dao.recommend.dao.imp.SettingDaoImp;
...
public class DaoRecommendManager extends AbstractBaseDao {
	...
	public ResponseMessageBean findMessage(int messageid) throws Exception {
		...
		connection = getConnection(); // Get connection
		List<flinn.old.dao.recommend.beans.response.ResponseMessageBean> messages = (new MessageDaoImp()
...
//			rtnList = new RuleDaoImp().findAllRules(bean, orderBy, connection);
	...
	public ResponseRuleBean findRule(int ruleid) throws Exception {
		...
		connection = getConnection(); // Get connection
//		List<flinn.old.dao.recommend.beans.response.ResponseRuleBean> rules = (new RuleDaoImp()
//				.find(bean, null, connection));
...
//		allDiagnoses = new RuleDaoImp().getAllDiagnoses(connection);
	...
	public ResponseSettingBean findSetting(int settingid) throws Exception {
		...
		connection = getConnection(); // Get connection
		List<flinn.old.dao.recommend.beans.response.ResponseSettingBean> settings = (new SettingDaoImp()
				.find(bean, null, connection));
