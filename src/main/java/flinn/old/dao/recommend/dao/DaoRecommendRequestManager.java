...
import flinn.old.dao.recommend.beans.RecommendPatientInfoBean;
...
//					RecommendDiagnosisBean[] diagnoses = new RuleDaoImp().getAllDiagnoses(connection);
					...
					int stage = flinn.old.dao.util.RecommendUtils.stageRecommendation(input.getInitialstaging().getDiagnosis(),input.getInitialstaging().getTreatments(), diagnoses);
