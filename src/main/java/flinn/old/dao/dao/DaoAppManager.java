...
import flinn.old.dao.beans.AppUserRoleBean;
import flinn.old.dao.beans.LabTestBean;
...
import flinn.old.dao.beans.request.RequestAppUserBean;
import flinn.old.dao.beans.request.RequestContainerBean;
import flinn.old.dao.beans.request.RequestFacilityBean;
import flinn.old.dao.beans.request.RequestPrescriptionBean;
import flinn.old.dao.beans.request.RequestTreatmentBean;
import flinn.old.dao.beans.response.ResponseActionBean;
import flinn.old.dao.beans.response.ResponseAppUserBean;
import flinn.old.dao.beans.response.ResponseAppUserContainerBean;
import flinn.old.dao.beans.response.ResponseContainerBean;
import flinn.old.dao.beans.response.ResponseFacilityBean;
import flinn.old.dao.beans.response.ResponseFacilityContainerBean;
import flinn.old.dao.beans.response.ResponseLabTestsContainerBean;
import flinn.old.dao.beans.response.ResponsePrescriptionBean;
import flinn.old.dao.beans.response.ResponseProgressNoteTagsContainerBean;
...
import flinn.old.dao.beans.response.ResponseTreatmentContainerBean;
import flinn.old.dao.beans.response.ResponseTreatmentGroupContainerBean;
import flinn.old.dao.beans.response.ResponseTreatmentSearchContainerBean;
...
public class DaoAppManager extends AbstractBaseDao
{
	...
	public ResponseFacilityBean getFacility(int facilityid) throws Exception
	{
		...
		if (facilityid > 0)
		{
			...
			List<flinn.old.dao.beans.response.ResponseFacilityBean> facilities = new FacilityDaoImp().find(facreq, null, connection);
	...
	public ResponseContainerBean updateFacility(RequestContainerBean input, ResponseSessionContainerBean session)
	{
		...
		//Force value of "0000-00-00 00:00:00" to avoid null exception in flinn.old.dao update
	...
	public LabTestBean findLabTest(int labtestid) throws Exception
	{
		...
		{
			connection = getConnection(); //Get connection
			List<flinn.old.dao.beans.LabTestBean> labtests = (new LabDaoImp().findTest(labtest, null, connection));
	...
	public TreatmentGroupBean getTreatmentGroup(int treatmentgroupid) throws Exception
	{
		...
		{
			connection = getConnection(); //Get connection
			List<flinn.old.dao.beans.TreatmentGroupBean> groups = (new TreatmentDaoImp().findGroup(group, null, connection));
	...
	public ResponsePrescriptionBean getPrescription(int prescriptionid) throws Exception
	{
		...
		if (prescriptionid > 0)
		{
			...
			flinn.old.dao.beans.request.RequestPrescriptionBean beanreq = new flinn.old.dao.beans.request.RequestPrescriptionBean();
			beanreq.setPrescriptionid(prescriptionid);
			List<flinn.old.dao.beans.response.ResponsePrescriptionBean> prescriptions = new PrescriptionDaoImp().find(beanreq, null, connection);
