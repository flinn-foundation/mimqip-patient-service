...
import flinn.old.dao.recommend.beans.RecommendTreatmentGuidelineBean;
	...
	public static boolean isConsistent(
		...
		for (int i=0; i<diagnosis.getTreatments().length; i++) {
			...
			if (rowworks) {
				...
				if (!rowworks) System.out.println("  flinn.flinn.old.dao.util.RecommendUtils.isConsistent  ---    rowworks false");
			}
		}
		return rowworks;
	}
