package flinn.old.dao.util;

import flinn.old.dao.beans.FacilityIPBean;
...
public class IpAddress {
	...
	public static final boolean validateFacilityIP(ResponseFacilityBean facility, HttpServletRequest req) {
		...
		long remoteL = -1;
		try {
			remoteL = flinn.old.dao.util.IpAddress.parse(remote);
		} catch (Exception e) {
		...
		if (fremote != null) {
			try {
				fremoteL = flinn.old.dao.util.IpAddress.parse(fremote);
			} catch (Exception e) {
